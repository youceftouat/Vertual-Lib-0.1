...
public class Bdd implements Signatures {
	private Map<Auteur, Set<Livre>> MapAuteur; //Map composé d'auteurs comme clef
 ...
 * de créer la map et ajouter les auteurs à la map.           *
 **************************************************************/
...
/*********************************************************************
 * Methode pour ajoute un auteur, passé en argument, dans la map.    *  
 * La partie valeur associée a cet est une nouvelle collection vide. * 
 *********************************************************************/
...
public void addLivre(Livre unLivre){
	...
	if(MapAuteur.containsKey(auteurLivre))
		...
		System.out.println("L'auteur de ce livre n'est pas dans la base de données");
}
...
public void rapportParAuteurs()
	...
	//Trier le map par ordre alphabétique des auteurs.
	MapTrier = trierMap(MapAuteur);
	try{
		fw = new FileWriter(nomACreer);			
	    } 
	catch (java.io.FileNotFoundException erreur){
	System.out.println("Problème de préparation de lecturl'écriture\n");
	probleme = true;
	}
	if (!probleme){
	...
	for (Map.Entry<Auteur, Set<Livre>> entry : MapTrier.entrySet()){
		...
		aCreer.close();
		System.out.println("Création du fichier " + nomACreer);				
		}
}
